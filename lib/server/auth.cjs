...
async function viewer(request) {
  ...
  const { rows } = await database().query('SELECT u.id,u.email,u.credits,u.email_verified_at FROM sessions s JOIN users u ON u.id=s.user_id WHERE s.token_hash=$1 AND s.expires_at>now()', [digest(token)]);
  if (!rows[0]) throw new HttpError(401, '登录已失效，请重新登录');
  if (process.env.PUBLIC_REGISTRATION === 'true' && !rows[0].email_verified_at) throw new HttpError(401, '请先验证邮箱');
  return rows[0];
}
...
async function authenticate(body, register) {
  ...
  if (register) {
    ...
    if (process.env.PUBLIC_REGISTRATION === 'true' && !row.email_verified_at) throw new HttpError(403, '请先验证邮箱');
...
async function hashPassword(password) {
  if (typeof password !== 'string' || password.length < 8 || password.length > 128) throw new HttpError(400, '请输入有效邮箱及 8—128 位密码');
  const salt = crypto.randomBytes(16).toString('hex');
  return `${salt}:${(await scrypt(password, salt, 64)).toString('hex')}`;
}
module.exports = { viewer, sameOrigin, authenticate, logout, throttle, digest, hashPassword };
