...
function publicJob(job) {
  ...
  return { taskId: job.id, kind: job.kind, stage: job.state === 'success' ? 'Success' : terminal ? 'Failure' : 'Processing',
    ...
    videoUrl: job.state === 'success' ? job.result?.videoUrl || null : null, result: job.state === 'success' ? job.result : null, error: job.error, state: job.state, updatedAt: new Date(job.updated_at).getTime() };
}
...
async function runOne(adapter, workerId) {
  ...
  try {
    ...
    if (err.identityBlocked) {
      await transition(job, 'needs_review', { error: err.message });
      return true;
    }
