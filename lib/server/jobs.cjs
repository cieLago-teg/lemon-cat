...
const { reserveCreation } = require('./creation-quota.cjs');
...
async function enqueue(userId, kind, key, input, requestId) {
  ...
  return transaction(async (client) => {
    ...
    const candidateCount = input.candidatesPerStyle ?? 1;
    if (kind === 'generate' && (!Number.isInteger(candidateCount) || candidateCount < 1 || candidateCount > 3)) throw new HttpError(400, '无效的候选数量');
    const creationId = await reserveCreation(client, userId, kind, input);
    const cost = quotaEnabled() ? 0 : COSTS[kind] * (kind === 'generate' ? candidateCount : 1);
    ...
    const storedInput = kind === 'generate' && Array.isArray(input.plan) ? { ...input, plan: input.plan.map((item) => ({ ...item, seed: crypto.randomInt(2147483648) })) } : input;
    const created = await client.query('INSERT INTO generation_jobs(id,user_id,kind,idempotency_key,input_hash,input,request_id,cost,creation_id) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *', [id,userId,kind,key,hash,storedInput,requestId,cost,creationId]);
...
function publicJob(job) {
  ...
  return { taskId: job.id, kind: job.kind, stage: job.state === 'success' ? 'Success' : terminal ? 'Failure' : 'Processing',
    ...
    videoUrl: job.state === 'success' ? job.result?.videoUrl || null : null, result: job.state === 'success' || (job.kind === 'generate' && job.result?.results?.length) ? job.result : null, error: job.error, state: job.state, updatedAt: new Date(job.updated_at).getTime() };
}
async function checkpointImages(job, result) {
  const saved = await database().query("UPDATE generation_jobs SET result=$1,updated_at=now() WHERE id=$2 AND locked_by=$3 AND lease_until>now() AND state='submitting'", [result,job.id,job.locked_by]);
  if (!saved.rowCount) throw new Error('Worker lease lost; image checkpoint not committed');
}
...
async function runOne(adapter, workerId) {
  ...
  try {
    ...
    // 后一张被明确拒绝，不代表前面已经生成的图片也没有计费。
    const partial = job.kind === 'generate' && err.safeToRefund
      ? (await getJob(job.user_id, job.id))?.result?.results?.length : 0;
    const state = job.state === 'submitting' ? (err.safeToRefund && !partial ? 'failed' : 'needs_review') : job.attempts >= 120 ? 'needs_review' : job.state;
...
module.exports = { enqueue, getJob, listJobs, publicJob, claim, transition, runOne, checkpointImages, COSTS };
