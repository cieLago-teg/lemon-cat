...
async function enqueue(userId, kind, key, input, requestId) {
  ...
  return transaction(async (client) => {
    ...
    const cost = COSTS[kind] * (kind === 'generate' ? candidateCount : 1);
    ...
    const created = await client.query('INSERT INTO generation_jobs(id,user_id,kind,idempotency_key,input_hash,input,request_id,cost) VALUES($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *', [id,userId,kind,key,hash,storedInput,requestId,cost]);
