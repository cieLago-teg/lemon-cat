...
const logger = pino({
  ...
  serializers: {
    err(error) {
      const clean = (value) => String(value || '').replace(/https?:\/\/\S+/g, '[URL]').replace(/(?:sk-|Bearer\s+)\S+/gi, '[REDACTED]').replace(/[^\s<>@]+@[^\s<>@]+\.[^\s<>@]+/g, '[EMAIL]');
      return { type: error?.name, message: clean(error?.message), stack: clean(error?.stack), code: error?.code,
        ...(error?.cause ? { cause: { type: error.cause.name, message: clean(error.cause.message), stack: clean(error.cause.stack), code: error.cause.code } } : {}) };
    }
  }
