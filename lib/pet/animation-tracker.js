...
function createAnimationTracker(options = {}) {
  /** @type {Map<string, {taskId: string, stage: string, percent: number, message: string, videoUrl?: string, error?: string, updatedAt: number}>} */
  const store = new Map();
  const persistence = options.filePath ? require('../db/json-store.cjs') : null;
  function persist() {
    if (persistence) persistence.writeJsonAtomic(options.filePath, [...store.values()]);
  }
  if (persistence) {
    const saved = persistence.readJson(options.filePath, []);
    if (!Array.isArray(saved)) throw new Error('Invalid animation task store; original data preserved.');
    for (const task of saved) {
      if (!task || typeof task.taskId !== 'string') throw new Error('Invalid animation task record.');
      if (task.stage !== 'Success' && task.stage !== 'Failure') {
        task.stage = 'Failure';
        task.error = '服务重启中断了任务。上游可能仍在处理，请核对任务后再重试，避免重复费用。';
        task.message = task.error;
        task.updatedAt = Date.now();
      }
      store.set(task.taskId, task);
    }
    // Single-process checkpoint only. This is not a distributed job queue.
    persist();
  }
  ...
  function setSubmitted(taskId, ownerId) {
    const s = ensure(taskId);
    if (s.ownerId && s.ownerId !== ownerId) throw new Error('Task owner cannot change');
    s.ownerId = ownerId;
    s.stage = "Submitted";
