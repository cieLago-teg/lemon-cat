...
test("integration: /api/pet/animate persists an authenticated job before returning", () => {
  ...
  assert.match(source, /requireOwnedImage/);
...
test("integration: deployed UI uses the shared bounded animation poller", () => {
  ...
  const path = require("node:path");
  const source = fs.readFileSync(
    path.join(__dirname, "..", "..", "app", "components", "useDeployPet.ts"),
    "utf8"
  );
  assert.match(source, /taskId/, "front-end must reference taskId for polling");
  assert.match(source, /pollAnimation/, "front-end must use the bounded poller");
