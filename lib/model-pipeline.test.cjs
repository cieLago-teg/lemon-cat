...
test('feedback requires four bounded ratings and strips untrusted provenance', () => {
  const { validateFeedback } = load('lib/feedback.ts');
  const valid = { ratings: { identity: 4, style: 5, anatomy: 4, desktop: 4 }, failures: ['crop', 'crop'], liked: true };
  assert.deepEqual(validateFeedback({ ...valid, model: 'forged', ratings: { ...valid.ratings, injected: 5 } }), { ...valid, failures: ['crop'] });
  for (const invalid of [null, {}, { ...valid, ratings: { ...valid.ratings, style: 6 } }, { ...valid, failures: ['toString'] }, { ...valid, liked: 'true' }]) assert.throws(() => validateFeedback(invalid), /评分/);
});

test('feedback endpoint binds owner and real job instead of client supplied provenance', async () => {
  const queries = []; let owned = true, found = true;
  const { POST } = load('app/api/image-feedback/route.ts', {
    '@/lib/server/http.cjs': { route: (_method, handler) => handler },
    '@/lib/server/guard': { requireUser: async () => ({ id: 'owner' }), requireOwnedImage: async () => { if (!owned) throw new Error('Not owned'); } },
    '@/lib/server/db.cjs': { database: () => ({ query: async (sql, params) => { queries.push({ sql, params }); return { rows: sql.startsWith('SELECT') && found ? [{ id: 'real-job' }] : [] }; } }) },
    '@/lib/feedback': load('lib/feedback.ts')
  });
  const payload = { imageUrl: '/api/assets/12345678-1234-1234-1234-123456789abc', ratings: { identity: 4, style: 4, anatomy: 4, desktop: 4 }, failures: [], liked: true, jobId: 'forged', model: 'forged' };
  const request = () => new Request('http://localhost/api/image-feedback', { method: 'POST', body: JSON.stringify(payload) });
  assert.equal((await POST(request())).status,200);
  assert.equal(queries[0].params[0],'owner');
  assert.deepEqual(queries[1].params,['owner',payload.imageUrl.slice(12),'real-job',{ ratings: payload.ratings, failures: [], liked: true }]);
  queries.length = 0; owned = false;
  await assert.rejects(POST(request()),/Not owned/);
  assert.equal(queries.length,0);
  owned = true; found = false;
  assert.equal((await POST(request())).status,404);
  assert.equal(queries.length,1,'unmatched asset cannot create feedback');
});
...
test('actual worker returns every generated style with its model provenance and no VL review', async (t) => {
  ...
  const mocks = {
    ...
    [path.join(root, 'lib/server/jobs.cjs')]: { checkpointImages: async () => {} },
    [path.join(root, 'lib/server/image-inspection.cjs')]: { inspectImage: async () => ({ warnings: [] }) },
  ...
  assert.deepEqual(result.results.map((item) => item.style), ['简约可爱水墨风', '粗描边贴纸风', '复古像素游戏风']);
  ...
  const checkpoints = [];
  // 使用真实计划：同画风多候选沿用确定的模型、提示词、seed 和负向约束。
  mocks[path.join(root, 'lib/server/jobs.cjs')] = { checkpointImages: async (_job, result) => checkpoints.push(JSON.parse(JSON.stringify(result))) };
  let attempts = 0;
  mocks['../lib/bailian'] = { generatePetImage: async (...args) => { calls.push(args); if (++attempts === 2) throw new Error('provider unavailable'); return 'https://dashscope-result.oss.aliyuncs.com/image.png'; } };
  const planned = load('scripts/worker.ts', mocks).adapter;
  const plan = [1, 2].map((candidate) => ({ style: '简约可爱水墨风', candidate, prompt: '已冻结提示词', model: models.DEFAULT_MODELS.image, seed: candidate, negativePrompt: '画外墨点', promptVersion: 'frozen' }));
  await assert.rejects(planned.submit({ ...job, input: { ...job.input, plan } }), /provider unavailable/);
  assert.equal(attempts, 2, '失败不自动重发付费调用');
  assert.equal(checkpoints.length, 1, '第一张图在第二张失败前已经保存');
  assert.equal(checkpoints[0].results[0].seed, 1);
  assert.equal(checkpoints[0].results[0].promptVersion, 'frozen');
  assert.equal(calls.at(-2)[4], '画外墨点');
});

test('generation selection is bounded and browser prompt overrides cannot replace server templates', () => {
  const { generationOptions, validateFeatureInput } = load('lib/generation-options.ts');
  assert.equal(generationOptions({}).total, 3);
  assert.equal(generationOptions({ candidatesPerStyle: 3 }).total, 9);
  assert.throws(() => generationOptions({ candidatesPerStyle: 100 }), /候选/);
  assert.throws(() => generationOptions({ styles: [] }), /画风/);
  assert.throws(() => generationOptions({ styles: ['和纸拼贴绘本风'] }), /画风/);
  assert.equal(generationOptions({ stylePrompts: [{ style: '简约可爱水墨风', template: 'override' }] }).styles[0].template, prompts.STYLE_PROMPTS[0].template);
  assert.throws(() => validateFeatureInput({ aiTags: [], customFeatures: '字'.repeat(121) }), /120/);
  assert.throws(() => validateFeatureInput({ aiTags: ['字'.repeat(41)] }), /精简/);
  assert.equal(models.buildPetImageRequest('p', 's', models.DEFAULT_MODELS.image, 1, prompts.styleNegativePrompt('复古像素游戏风')).parameters.negative_prompt.includes('抗锯齿'), true);
});
