...
const { resolveLocale, messages, translate, authError, photoMessage } = compiled.exports;
...
test('photo warnings translate known system text without modifying unknown content', () => {
  assert.match(photoMessage('en', '框选区域太小，请扩大到完整宠物'), /crop is too small/);
  assert.equal(photoMessage('zh', '分辨率偏低，建议选择更清晰的原图'), '分辨率偏低，建议选择更清晰的原图');
  assert.equal(photoMessage('en', 'an-unrecognized-diagnostic'), 'an-unrecognized-diagnostic');
});
