...
import { requireVerifiedImage } from '@/lib/server/identity.cjs';
export const POST = route('POST', async (request, _context, { requestId }) => {
  ...
  await requireVerifiedImage(user.id, body.imageUrl);
  if (body.sourceImageUrl && body.sourceImageUrl !== body.imageUrl) return NextResponse.json({ error: '视频必须使用通过质检的原始形象，不支持替换参考图' }, { status: 422 });
