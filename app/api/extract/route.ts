...
export const POST = route('POST', async (request, _context, { requestId }) => {
  ...
  await inspectImage(Buffer.from(body.imageBase64, 'base64'), body.mimeType);
  const job = await enqueue(user.id, 'extract', request.headers.get('idempotency-key') || '', { imageBase64: body.imageBase64, mimeType: body.mimeType }, requestId);
