...
export default class ErrorBoundary extends Component<Props, State> {
  ...
  render() {
    ...
    return <ErrorFallback message={this.state.message} retry={() => this.setState({ hasError: false, message: '' })} />;
  }
}

function ErrorFallback({ message, retry }: { message: string; retry: () => void }) {
  const { t } = useLocale();
  return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/70 px-6 backdrop-blur-sm">
        <div className="w-full max-w-md rounded-3xl border border-amber-200/70 bg-white/90 p-6 text-center shadow-[0_18px_45px_-24px_rgba(92,46,16,0.45)]">
          <p className="font-handwriting text-xl font-bold text-amber-900">{t('crashTitle')} 🐾</p>
